...
// Compute raw days integer (for averaging logic)
const getRawDays = (datePurchase, dateSold) => {
  if (!datePurchase || !dateSold) return null;
  const start = new Date(datePurchase);
  const end = new Date(dateSold);
...
// ==========================================
// 3. GROUP TOGGLE LOGIC (ACCORDION)
// ==========================================
window.toggleGroup = function(groupId) {
  ...
  if (icon) {
    ...
    icon.style.color = isExpanded ? '#4ade80' : 'var(--gold-primary)';
  }
};

// ==========================================
// 4. RENDER TABLE FUNCTION (GROUPED & SORTED)
// ==========================================
function renderAnalyticsTable(productsList) {
  const tbody = document.getElementById('analyticsList');
  if (!tbody) return;
  ...
  productsList.forEach(item => {
    const rawName = item.productName || 'Unnamed Product';
    ...
    if (item.price && !isNaN(item.price)) {
      group.totalCapital += Number(item.price);
      group.validCapitalCount++;
    }

    // Status and Days computing
    if (item.status === 'Sold') {
      group.soldCount++;
      const days = getRawDays(item.datePurchase, item.dateSold);
      if (days !== null) group.totalSoldDays += days;
    } else {
      group.availableCount++;
    }
  });

  // 2. COMPUTE AVERAGES & CONVERT TO ARRAY FOR SORTING
  const groupsArray = Object.keys(groupedData).map(key => {
    const g = groupedData[key];
    g.avgCapital = g.validCapitalCount > 0 ? (g.totalCapital / g.validCapitalCount) : 0;
    // Kung walang benta, infinity para mapunta sa pinakababa sa sorting
    g.avgDays = g.soldCount > 0 ? (g.totalSoldDays / g.soldCount) : Infinity; 
    return { id: key.replace(/[^a-zA-Z0-9]/g, ''), ...g }; // generate safe ID
  });

  // 3. SORT GROUPS (Fastest to Sell -> Slowest -> No Sales)
  groupsArray.sort((a, b) => a.avgDays - b.avgDays);

  let htmlContent = '';

  // 4. RENDER HTML (Master Row + Hidden Child Rows)
  groupsArray.forEach((group, index) => {
    const groupId = `group-${group.id}-${index}`;
    const totalUnits = group.items.length;
    
    // Formatting Averages
    const avgDaysDisplay = group.avgDays === Infinity 
      ? `<span style="color: #aaa;">No Sales Yet</span>` 
      : `<span style="color: #4ade80;">${Math.round(group.avgDays)} days avg</span>`;
      
    const stockStatus = `<span style="color: #4ade80;">${group.soldCount} Sold</span> | <span style="color: var(--gold-primary);">${group.availableCount} Avail</span>`;

    // 🔴 MASTER ROW (Clickable Accordion Header)
    htmlContent += `
      <tr style="cursor: pointer; background: rgba(226, 178, 88, 0.05); border-bottom: 2px solid rgba(255,255,255,0.1);" onclick="toggleGroup('${groupId}')">
        <!-- 1. Master: Name & Toggle -->
        <td style="width: 25%; padding: 10px 4px; vertical-align: middle;">
          <div style="font-size: clamp(0.7rem, 2.2vw, 0.85rem); font-weight: 700; color: #fff;">
            <span id="icon-${groupId}" style="display: inline-block; width: 14px; color: var(--gold-primary); transition: 0.2s;">▶</span>
            ${group.displayName}
          </div>
          <div style="margin-top: 2px; font-size: 0.65rem; color: #aaa; margin-left: 18px;">
            Total Units: ${totalUnits}
          </div>
        </td>

        <!-- 2. Master: Avg Capital -->
        <td style="width: 20%; padding: 10px 2px; font-size: clamp(0.65rem, 2vw, 0.75rem); vertical-align: middle; text-align: center; color: #ccc;">
          <small>Avg Capital</small><br>
          <b style="color: #fff;">${formatCurrency(group.avgCapital)}</b>
        </td>

        <!-- 3. Master: Empty/Placeholder (Para di siksik) -->
        <td style="width: 15%; padding: 10px 2px; vertical-align: middle; text-align: center;">
           —
        </td>

        <!-- 4. Master: Stock Status (Avail/Sold) -->
        <td style="width: 20%; padding: 10px 2px; font-size: clamp(0.65rem, 2vw, 0.75rem); vertical-align: middle; text-align: center;">
          ${stockStatus}
        </td>

        <!-- 5. Master: Speed Indicator (Avg Days) -->
        <td style="width: 20%; padding: 10px 2px; font-size: clamp(0.65rem, 2vw, 0.75rem); font-weight: 700; vertical-align: middle; text-align: center;">
          ⚡ ${avgDaysDisplay}
        </td>
      </tr>
    `;

    // 🔵 CHILD ROWS (Individual Items, initially hidden)
    // Sort items inside group: Sold muna, tapos Available
    ...
    sortedItems.forEach((item, itemIndex) => {
      const key = item.id;
      const buyerName = item.buyerName || '—';
      const daysSoldInfo = calculateDaysDifference(item.datePurchase, item.dateSold);
      
      const statusBadge = item.status === "Sold" 
        ? `<span style="background: rgba(74, 222, 128, 0.15); color: #4ade80; padding: 1px 4px; border-radius: 3px; font-size: 0.55rem; font-weight: 700;">SOLD</span>`
        : `<span style="background: rgba(226, 178, 88, 0.15); color: var(--gold-primary); padding: 1px 4px; border-radius: 3px; font-size: 0.55rem; font-weight: 700;">AVAILABLE</span>`;

      // Isang "L" na arrow para magmukhang nested
      const isLast = itemIndex === sortedItems.length - 1;
      const borderStyle = isLast ? 'border-bottom: 1px solid rgba(255,255,255,0.05);' : 'border-bottom: 1px dashed rgba(255,255,255,0.05);';

      htmlContent += `
        <tr class="${groupId}-child" style="display: none; cursor: pointer; background: rgba(0,0,0,0.3); ${borderStyle}" onclick="openProductDetailModal('${key}')">
          
          <td style="width: 25%; padding: 6px 4px 6px 18px; vertical-align: middle; word-break: break-word;">
          ...
          <td style="width: 20%; padding: 6px 2px; font-size: 0.7rem; vertical-align: middle; text-align: center; color: #ccc;">
            ${formatCurrency(item.price)}
          </td>

          <td style="width: 15%; padding: 6px 2px; font-size: 0.7rem; color: var(--gold-primary); vertical-align: middle; text-align: center;">
            ${formatCurrency(item.sellingPrice)}
          </td>

          <td style="width: 20%; padding: 6px 2px; font-size: 0.7rem; color: #fff; vertical-align: middle; text-align: center;">
            ${buyerName}
          </td>

          <td style="width: 20%; padding: 6px 2px; font-size: 0.7rem; color: #4ade80; vertical-align: middle; text-align: center;">
            ${item.status === 'Sold' ? daysSoldInfo : '—'}
          </td>
        </tr>
      `;
    });
...
export function filterProductsBySearch(query) {
  ...
  // 2. CHECK SPECIFIC VARIANT CRITERIA (Retained original logic)
  const tokens = searchTerm.split(/\s+/).filter(t => t.length > 0);
  const hasSpecificSpec = tokens.some(token => {
    return /^i[3579]$/.test(token) ||                    
           /^r[3579]$/.test(token) ||                    
           /^\d+(\+\d+)?\/\d+$/.test(token) ||            
           /^(64|128|256|512|1tb)$/.test(token) ||        
