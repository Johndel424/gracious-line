...
export function showChartLoading() {
  ...
  if (totalBadge) {
    totalBadge.textContent = 'Loading chart...';
  }

  // 1. Clear muna ang dots
  if (dotsGroup) dotsGroup.innerHTML = '';

  // 2. Maglagay ng animated temporary SVG path (wave placeholder)
  const placeholderPath = "M0,150 Q125,80 250,130 T500,100";
  const placeholderArea = "M0,150 Q125,80 250,130 T500,100 L500,180 L0,180 Z";

  if (chartPath) {
    chartPath.setAttribute('d', placeholderPath);
    chartPath.style.opacity = '0.3';
  }
  
  if (chartArea) {
    chartArea.setAttribute('d', placeholderArea);
    chartArea.style.opacity = '0.15';
  }

  // 3. Temporary Grid Lines
  if (gridGroup) {
    gridGroup.innerHTML = `
      <line x1="0" y1="30" x2="500" y2="30" stroke="rgba(226,178,88,0.08)" stroke-dasharray="3" />
      <line x1="0" y1="105" x2="500" y2="105" stroke="rgba(226,178,88,0.08)" stroke-dasharray="3" />
      <line x1="0" y1="180" x2="500" y2="180" stroke="rgba(226,178,88,0.08)" stroke-dasharray="3" />
    `;
  }
}
// ==========================================
...
window.handleTimeframeChange = function(months) {
  ...
  // 🔴 1. IPAKITA ANG CHART LOADING HABANG NAGPA-PALIT NG TIMEFRAME
  if (typeof showChartLoading === 'function') {
    showChartLoading();
  }

  // Re-render ang chart pagkatapos ng maikling transition
  setTimeout(() => {
    renderPerformanceChart();
  }, 150);
};
...
export function renderPerformanceChart() {
  // 🔴 2. KAPAG WALA PANG DATA, IPAKITA MUNA ANG SKELETON
  if (!productsDataStore || Object.keys(productsDataStore).length === 0) {
    if (typeof showChartLoading === 'function') showChartLoading();
    return;
  }
...
// 🔴 3. TAWAGIN ANG LOADING BAGO MAG-FIREBASE FETCH!
